...
if (!function_exists('osdd')) {
    function osdd($var, $label = NULL, $stacktrace = TRUE) {
        $osdebug = new OSDebug;
        echo $osdebug->osd($var, $label, $stacktrace);
        die;
    }
...
if (!function_exists('osv')) {
    function osv($var, $pre = TRUE) {
        echo $pre ? '<pre>' : '';
        echo var_export($var);
        echo $pre ? '</pre>' : '';
    }
...
if (!function_exists('osvd')) {
    function osvd($var, $pre = TRUE) {
        echo $pre ? '<pre>' : '';
        echo var_export($var);
        echo $pre ? '</pre>' : '';
        die;
...
class OSDTImer {
    ...
    /**
     * Start a previously closed timer (old duration + more)
     *
     * @param $index
     */
    public function restart($index)
    {
        ...
        $this->start[$index] = microtime(TRUE) - $interval;
